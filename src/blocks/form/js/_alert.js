...
import { removeChildren, makeHumanReadable } from '../../../common/_util'
...
const alertsShow = async ( form, alerts ) => {
	...
	const container = form.querySelector( '.bigupForms__alertsContainer' )
	const output    = form.querySelector( '.bigupForms__alerts' )
	if ( ! container || ! output ) return

	// Show.
	if( debug ) console.log( `${stopwatch()} |START| alertsShow | ${alerts[ 0 ][ 'text' ]}` )
	formLock( form, true )
	container.style.display = 'flex'
	await transition( container, 'opacity', '0' )
	await removeChildren( output )
	await popoutsIntoDom( output, alerts )
	await transition( container, 'opacity', '1' )
	return( 'alert alertsShow complete' )
}


/**
 * Update, wait then hide alerts in a form.
 * 
 * @param {object} form The target form.
 * @param {array} alerts Alert objects to be displayed.
 * @param {int} wait Time to wait.
 */
const alertsUpdateWaitHide = async ( form, alerts, wait ) => {
	...
	// Update.
	if( debug ) console.log( `${stopwatch()} |START| alertsUpdateWaitHide | ${alerts[ 0 ][ 'text' ]}` )
	formLock( form, true )
	await removeChildren( output )
	await popoutsIntoDom( output, alerts )
	// Wait.
	...
	formLock( form, false )
	if( debug ) console.log( `${stopwatch()} | END | alertsUpdateWaitHide | ${alerts[ 0 ][ 'text' ]}` )
	return( 'alert alertsUpdateWaitHide complete' )
}
...
const alertsShowWaitHide = async ( form, alerts, wait ) => {
	...
	// Show.
	if( debug ) console.log( `${stopwatch()} |START| alertsShowWaitHide | ${alerts[ 0 ][ 'text' ]}` )
	formLock( form, true )
	container.style.display = 'flex'
	await transition( container, 'opacity', '0' )
	await removeChildren( output )
	...
	container.style.display = 'none'
	formLock( form, false )
	if( debug ) console.log( `${stopwatch()} | END | alertsShowWaitHide | ${alerts[ 0 ][ 'text' ]}` )
...
function popoutsIntoDom( output, alerts ) {
	...
	if( debug ) console.log( `${stopwatch()} |START| popoutsIntoDom | ${alerts[ 0 ][ 'text' ]}` )
	return new Promise( ( resolve, reject ) => {
		try {
			...
			if( debug ) console.log( `${stopwatch()} | END | popoutsIntoDom | ${alerts[ 0 ][ 'text' ]}` )
		}
	} )
}
...
export { alertsShow, alertsUpdateWaitHide, alertsShowWaitHide }
